# 英語カラオケアプリ ハイライト機能改善 技術調査レポート

## エグゼクティブサマリー

高速モードでハイライトが追従せず巻き戻る問題は、**Chrome Web Speech APIの根本的な制約**、つまり**単語レベルのタイムスタンプ情報が提供されない**ことに起因しています。本調査の結果、この問題を解決するには、音声認識をタイミング制御から分離し、代替技術を組み合わせたハイブリッドアーキテクチャが必要であることが判明しました。

最も効果的なアプローチは、**事前処理で歌詞タイムスタンプを生成し、リアルタイム処理ではIncremental DTWとAudioContext時計を使用する方法**です。これにより92.26%の精度と200ms以下のレイテンシを実現できます。
